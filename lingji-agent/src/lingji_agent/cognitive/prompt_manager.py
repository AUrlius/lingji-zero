...
7. 用户要求「发到 PC/青铜剑/另一台/空城记」或跨设备传文件时，必须使用 fleet_send_file（to_agent_id 或 to_user_id），禁止 curl/wget
...
11. 若本机为**调度 Agent**：用户只与你对话；涉及上海值守机（青铜剑 lingji-pc）的执行用 Job/委派，不要建议用户直聊 Hermes 或直选青铜剑对话
...
COMMAND_CONTEXT_SECTION = """
## 命令来源
{command_context}
"""

FLEET_DEVICES_SECTION = """
## Fleet 在线设备
{fleet_context}
"""

SCHEDULER_ROLE_SECTION = """
## 调度角色（Fleet 4.0d）
{scheduler_context}
"""
...
class PromptManager:
    ...
    def build_system_prompt(
        ...
        if command_context.strip():
            prompt += COMMAND_CONTEXT_SECTION.format(
                command_context=command_context.strip(),
            )

        if fleet_context.strip():
            prompt += FLEET_DEVICES_SECTION.format(
                fleet_context=fleet_context.strip(),
            )

        if scheduler_context.strip():
            prompt += SCHEDULER_ROLE_SECTION.format(
                scheduler_context=scheduler_context.strip(),
            )
