...
(function () {
  ...
  var GATEWAY_TOKEN = 'lingji-b21aaff8';
  ...
  function enqueueHitlRes(taskId, decision) {
    ...
    queue.push({ task_id: taskId, decision: decision, ts: Date.now() });
  ...
  function flushHitlResQueue() {
    ...
    queue.forEach(function (item) {
      ws.send(JSON.stringify({
        ...
        payload: { task_id: item.task_id, decision: item.decision },
      }));
    });
    saveHitlResQueue([]);
  ...
  function showHitlRequest(payload) {
    UI().showHitlCard(payload, function (taskId, decision) {
      ...
      ws.send(JSON.stringify({
        ...
        device_id: DEVICE_ID,
        timestamp: Date.now() / 1000,
        payload: { task_id: taskId, decision: decision },
      }));
    });
  }
