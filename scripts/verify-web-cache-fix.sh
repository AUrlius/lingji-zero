...
JS=$(curl -sf "https://${HOST}/js/lingji-api.js?v=0.1.4")
...
if ! echo "$JS" | grep -q 'CONNECTION_ID'; then
  echo "FAIL: missing Fleet multi-connect CONNECTION_ID"
  exit 1
fi
echo "OK: new JS (token + user_id + multi-connect)"

echo "[verify] index.html script tags"
IDX=$(curl -sf "https://${HOST}/")
echo "$IDX" | grep -q 'lingji-api.js?v=0.1.4' || { echo "FAIL: index missing v=0.1.4"; exit 1; }
